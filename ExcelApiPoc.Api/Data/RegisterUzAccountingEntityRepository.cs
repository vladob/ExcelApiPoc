...
public sealed class RegisterUzAccountingEntityRepository
{
    ...
    public async Task<RegisterUzAccountingEntityGraph?> GetByIcoAsync(string ico, CancellationToken cancellationToken = default)
    {
    ...
    private static async Task<AccountingEntityDto?> ReadEntityAsync(SqlConnection connection, string ico, CancellationToken cancellationToken)
    {
        const string sql =
            """
            SELECT
    ...
    private static async Task<List<FinancialReportRow>> ReadFinancialReportsAsync(SqlConnection connection, long entityId, CancellationToken cancellationToken)
    {
        const string sql =
            """
        SELECT
    ...
    private static async Task<Dictionary<long, FinancialReportTitlePageDto>> ReadFinancialReportTitlePagesAsync(SqlConnection connection, long entityId, CancellationToken cancellationToken)
    {
        const string sql =
            """
        SELECT
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            result[financialReportId] =
                new FinancialReportTitlePageDto
                {
                    ...
                    CompletionDate =
                        GetNullableDateOnly(reader, 20),

                    ApprovalDate =
                        GetNullableDateOnly(reader, 21),

                    PreparationDate =
                        GetNullableDateOnly(reader, 22),

                    AssemblyDate =
                        GetNullableDateOnly(reader, 23),
    ...
    private static async Task<Dictionary<long, List<FinancialReportAttachmentDto>>> ReadFinancialReportAttachmentsAsync(SqlConnection connection, long entityId, CancellationToken cancellationToken)
    {
        const string sql =
            """
        SELECT
    ...
    private static async Task<IReadOnlyList<FinancialStatementDto>> ReadFinancialStatementsAsync(SqlConnection connection, long entityId, IReadOnlyDictionary<long, List<FinancialReportDto>> reportsByStatementId, CancellationToken cancellationToken)
    {
        const string sql =
            """
            SELECT
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            result.Add(
                new FinancialStatementDto
                {
                    ...
                    SubmissionDate =
                        GetNullableDateOnly(reader, 3),
                    PreparationDate =
                        GetNullableDateOnly(reader, 4),
                    ApprovalDate =
                        GetNullableDateOnly(reader, 5),
                    AssemblyDate =
                        GetNullableDateOnly(reader, 6),
                    AuditorReportAttachmentDate =
    ...
    private static async Task<IReadOnlyList<AnnualReportDto>> ReadAnnualReportsAsync(SqlConnection connection, long entityId, IReadOnlyDictionary<long, List<FinancialReportDto>> reportsByAnnualReportId, CancellationToken cancellationToken)
    {
        Dictionary<long, List<AnnualReportAttachmentDto>> attachments = await ReadAnnualReportAttachmentsAsync( connection, entityId, cancellationToken);
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            result.Add(
                new AnnualReportDto
                {
                    ...
                    // Attachments are added in the next repository slice.
                    Attachments =
                        attachments.TryGetValue(
                            annualReportId,
    ...
    Dictionary<long, List<AnnualReportAttachmentDto>>> ReadAnnualReportAttachmentsAsync(SqlConnection connection, long entityId, CancellationToken cancellationToken)
    {
        const string sql =
            """
        SELECT
